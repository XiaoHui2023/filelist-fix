...
| `--debug-dump` |  | 目录路径 |  | 将每个源文件依赖解析流水线各阶段文本写入该目录下 `by_file/…`；**每次运行先清空 `by_file/` 再写入**，仅保留本轮产物；根目录仅 **README.txt**；`by_file` 子目录名为短文件名 + 下划线 + 源绝对路径 **SHA256 前 16 位十六进制**（路径指纹）；与 **`-l`** 无关；解析缓存命中且未重算时不会为该源写片段 |
